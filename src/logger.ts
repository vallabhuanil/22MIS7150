...
import { Stack, Level, Package, LogPayload, LogResponse } from './types.js';
import { config, validateConfig } from './config.js';

const ALLOWED_STACKS: Stack[] = ['backend', 'frontend'];
const ALLOWED_LEVELS: Level[] = ['debug', 'info', 'warn', 'error', 'fatal'];
const ALLOWED_PACKAGES: Package[] = [
...
export async function Log(stack: Stack, level: Level, pkg: Package, message: string): Promise<LogResponse | null> {
  ...
  try {
    const response = await axios.post<LogResponse>(config.endpoint, payload, {
      headers: {
